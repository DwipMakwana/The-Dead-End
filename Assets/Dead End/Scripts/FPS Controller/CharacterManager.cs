...
public class CharacterManager : MonoBehaviour 
{
    ...
    [SerializeField] private GameObject         _bulletHoles;
	...
	public void DoDamage( int hitDirection = 0 )
	{
        ...
        isSomethingHit = Physics.Raycast(ray, out hit, _range, 1 << _aiBodyPartLayer);

        if (GameObject.FindGameObjectWithTag("Weapon2") == true)
        {
            _range = 10f;
        }
        else
		...
		if (isSomethingHit)
		{
			...
			if (stateMachine)
			{
                if (GameObject.FindGameObjectWithTag("Weapon2") == true)
                {
                    stateMachine.TakeDamage(hit.point, ray.direction * 100.0f, 1, hit.rigidbody, this, 0);
                    _nextAttackTime = Time.time + 2.0f;
	...
	void Update()
	{
        ...
        float rayLength = Mathf.Lerp(0.0f, 20f, Mathf.Abs(Vector3.Dot(_camera.transform.forward, Vector3.up)));
		...
		// Update the Health and Stamina on the Player HUD
	...
	{
		if (_fpsController) 
			_fpsController.freezeMovement = false;
